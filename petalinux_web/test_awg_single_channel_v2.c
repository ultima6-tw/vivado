// test_awg.c — A.ch0 toggle 1k/20k, using gain&commit / index&commit
#include <stdio.h>
#include <stdint.h>
#include <time.h>
#include "awg_core.h"
...
static inline uint32_t make_index_commit_word(int ch, int tone, uint32_t idx20) {
    return (0x4u << 28) | pack_sel(ch, tone) | (idx20 & 0xFFFFFu);
}
// 0x3 = GAIN&COMMIT
static inline uint32_t make_gain_commit_word(int ch, int tone, uint32_t g20) {
    return (0x3u << 28) | pack_sel(ch, tone) | (g20 & 0xFFFFFu);
}
...
int main(void) {
    ...
    printf("Loop: A.tone0 -> 1k(max) [gain&commit] -> 20k(max) [gain&commit] -> repeat\n");

    for (;;) {
        // ---- Set 1 kHz on A.tone0 ----
        {
            uint32_t w[2];
            w[0] = make_index_commit_word(CH_A, TONE_0, IDX_1K);
            w[1] = make_gain_commit_word (CH_A, TONE_0, GAIN_Q1_17_MAX);
            awg_send_words32(w, 2);
        }
        sleep_ns(GAP_NS);

        // ---- Set 20 kHz on A.tone0 ----
        {
            uint32_t w[2];
            w[0] = make_index_commit_word(CH_A, TONE_0, IDX_20K);
            w[1] = make_gain_commit_word (CH_A, TONE_0, GAIN_Q1_17_MAX);
            awg_send_words32(w, 2);
        }
        sleep_ns(GAP_NS);
